...
export const loader = async ({ request }: LoaderFunctionArgs) => {
    ...
    const appPlan = (settings as { appPlan?: string } | null)?.appPlan;
    const isProPlan = appPlan === "pro";
    if (!isProPlan && customerApprovalSettings) {
        customerApprovalSettings = { ...customerApprovalSettings, approvalMode: "manual" };
    }
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    const appSettingsRow = await prisma.appSettings.findUnique({ where: { shop }, select: { appPlan: true } });
    const isProPlan = appSettingsRow?.appPlan === "pro";
    if (!isProPlan) {
        settingsToPersist = { ...settingsToPersist, approvalMode: "manual" };
    }
...
export default function Settings() {
    const {
        ...
        isProPlan: initialIsProPlan = false,
    ...
    useEffect(() => {
        ...
        async function loadTemplateSelection() {
            try {
                const response = await authenticatedFetch("/app/api/template-selection", {
                    method: "GET",
    ...
    const handleSave = async () => {
        try {
            await authenticatedFetch("/app/api/template-selection", {
                method: "POST",
                                ...
                                <BlockStack gap="400">
                                    <ChoiceList
                                        ...
                                        choices={[
                                            ...
                                            ...(isProPlan ? [{ label: "Auto approval", value: "auto" as const }] : []),
                                        ]}
                                        selected={[isProPlan ? customerApprovalSettings.approvalMode : "manual"]}
                                        onChange={(selected) =>
                                            setCustomerApprovalSettings((prev) => ({
                                                ...prev,
                                                approvalMode: (isProPlan ? (selected[0] as "manual" | "auto") : "manual") || "manual",
                                            }))
                                        }
                                    />
                                    {!isProPlan && (
                                        <Text as="p" tone="subdued">
                                            Auto approval is available on the Pro plan. Upgrade in Pricing to enable it.
                                        </Text>
                                    )}
                                    <TextField
