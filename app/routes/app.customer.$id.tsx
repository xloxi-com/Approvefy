...
import {
  ...
  buildRegistrationAdminLayout,
  getRegistrationFormFieldsForShopWithAdmin,
...
const REGISTRATION_COLUMN_FIELDS = new Set([
  "firstName",
  "lastName",
  "email",
  "phone",
...
/** Shared parsing for customData values (text vs file metadata) on the customer detail form. */
function parseCustomDataValueForAdmin(value: unknown): {
  fileList: FileEntry[];
  isFileField: boolean;
  plainText: string;
} {
  ...
  return { fileList, isFileField, plainText };
}
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  ...
  try {
    ...
    const phoneValidationError = validateStoredRegistrationPhone(phoneRaw);
    if (phoneValidationError) {
      return { success: false, error: phoneValidationError };
    }
    const phoneNormalized = normalizeRegistrationPhone(phoneRaw) || null;
...
export default function CustomerDetailPage() {
  ...
  useEffect(() => {
    ...
    setPhoneError(validateStoredRegistrationPhone(loadedPhone));
    ...
    if (cd || data.registrationLayout.length > 0) {
      const initial: Record<string, string> = {};
      const obj = cd ?? {};
      if (data.registrationLayout.length === 0) {
        Object.entries(obj).forEach(([key, value]) => {
          if (value == null) return;
          const normalized = normalizeCustomDataValueForText(value);
          if (normalized !== "") initial[key] = normalized;
        });
      } else {
        Object.entries(obj).forEach(([key, value]) => {
          initial[key] = normalizeCustomDataValueForText(value);
        });
        for (const item of data.registrationLayout) {
          if (item.kind !== "field") continue;
          if (REGISTRATION_COLUMN_FIELDS.has(item.apiKey)) continue;
          if (!(item.apiKey in initial)) initial[item.apiKey] = "";
        }
      }
  ...
  const hasUnsavedChanges = useMemo(() => {
    ...
    const keys = new Set([...Object.keys(baseCd), ...Object.keys(customDataValues)]);
    for (const key of keys) {
      const original = normalizeCustomDataValueForText(baseCd[key]);
      if ((customDataValues[key] ?? original) !== original) return true;
    }
    return false;
  ...
  const handleDiscard = useCallback(() => {
    ...
    if (data.registrationLayout.length === 0) {
      ...
      const reset: Record<string, string> = {};
      Object.entries(cd).forEach(([key, value]) => {
        reset[key] = normalizeCustomDataValueForText(value);
      });
      for (const item of data.registrationLayout) {
        if (item.kind !== "field") continue;
        if (REGISTRATION_COLUMN_FIELDS.has(item.apiKey)) continue;
        if (!(item.apiKey in reset)) reset[item.apiKey] = "";
      }
      setCustomDataValues(reset);
    }
  }, [data]);
  ...
  const handlePhoneChange = (value: string) => {
    const cleaned = normalizeRegistrationPhone(value);
    setPhone(cleaned);
    setPhoneError(validateStoredRegistrationPhone(cleaned));
  };
  ...
  const layoutApiKeys = useMemo(() => {
    ...
    return new Set(
      data.registrationLayout.filter((i) => i.kind === "field").map((i) => i.apiKey)
    );
  }, [data]);

  const useLegacyRegistrationLayout = useMemo(() => {
    if (!data || "error" in data) return true;
    return data.registrationLayout.length === 0;
  }, [data]);

  const orphanCustomDataEntries = useMemo(() => {
    if (!data || "error" in data) return [];
    if (data.registrationLayout.length === 0) return [];
    if (!data.customData || typeof data.customData !== "object" || Array.isArray(data.customData)) {
      return [];
    }
    return Object.entries(data.customData as Record<string, unknown>).filter(
      ([k]) => !layoutApiKeys.has(k)
    );
  }, [data, layoutApiKeys]);

  const renderOneCustomDataEntry = (key: string, value: unknown) => {
    if (!data || "error" in data) return null;
    const { fileList, isFileField, plainText } = parseCustomDataValueForAdmin(value);
        ...
        {isFileField ? (
          <BlockStack gap="200">
            {fileList.map((file, idx) => {
              const fname = file.name || `File ${idx + 1}`;
                ...
                >
                  <InlineStack gap="400" align="space-between" blockAlign="center">
                    ...
                    <ButtonGroup variant="segmented">
                      {file.url ? (
                        <>
                          <Button size="slim" url={file.url} target="_blank">
                            View
                          </Button>
                          <Button size="slim" onClick={() => handleDownload(file.url!, fname)}>
                            Download
                          </Button>
                        </>
                      ) : (
          ...
          <SectionCard title="Registration details">
            <BlockStack gap="400">
              {useLegacyRegistrationLayout ? (
                ...
                <>
                  {data.registrationLayout.map((item, layoutIdx) => {
                    if (item.kind === "heading") {
                      return (
                        <BlockStack key={`heading-${layoutIdx}`} gap="100">
                    ...
                    const { apiKey, label, helpText, fieldType, options } = item;
                    if (fieldType === "password") return null;

                    if (apiKey === "firstName") {
                      return (
                        <TextField
                          key={`fld-${layoutIdx}-${apiKey}`}
                          label={label}
                          helpText={helpText}
                          name="firstName"
                          value={firstName}
                    ...
                    if (apiKey === "email") {
                      return (
                        <Fragment key={`fld-${layoutIdx}-${apiKey}`}>
                          <input type="hidden" name="email" value={email} />
                          <TextField
                            label={label}
                            helpText={
                              helpText?.trim()
                                ? helpText
                                : "Email cannot be changed from the admin. Ask the customer to update it in their Shopify account."
                            }
                            type="email"
                            value={email}
                            autoComplete="email"
                    ...
                    const rawCustom = data.customData?.[apiKey];
                    if (fieldType === "email") {
                      const lockedEmailValue =
                        customDataValues[apiKey] ?? normalizeCustomDataValueForText(rawCustom);
                      return (
                        <Fragment key={`fld-${layoutIdx}-${apiKey}`}>
                          <input type="hidden" name={`customData.${apiKey}`} value={lockedEmailValue} />
                          <TextField
                            label={label}
                            helpText={
                              helpText?.trim()
                    ...
                    if (fieldType === "file_upload") {
                      return (
                        <Fragment key={`fld-${layoutIdx}-${apiKey}`}>
                          {renderOneCustomDataEntry(apiKey, rawCustom)}
                        </Fragment>
                      );
                    }
                    ...
                    const selectOptions =
                      ...
                      options &&
                      options.length > 0
                        ? [{ label: "—", value: "" }, ...options.map((o) => ({ label: o, value: o }))]
                        : null;
                    ...
                    if (selectOptions) {
                      const v =
                        customDataValues[apiKey] ?? normalizeCustomDataValueForText(rawCustom);
                      return (
                        <BlockStack key={`fld-${layoutIdx}-${apiKey}`} gap="200">
                          <input type="hidden" name={`customData.${apiKey}`} value={v} />
                          <Select
                            label={label}
                            helpText={helpText}
                    ...
                    const parsed = parseCustomDataValueForAdmin(rawCustom);
                    if (parsed.isFileField) {
                      return (
                        <Fragment key={`fld-${layoutIdx}-${apiKey}`}>
                          {renderOneCustomDataEntry(apiKey, rawCustom)}
                        </Fragment>
                    ...
                    let displayVal = customDataValues[apiKey] ?? parsed.plainText;
                    if (
                      typeof displayVal === "string" &&
                      displayVal.length === 2 &&
                      /^[A-Z]{2}$/.test(displayVal) &&
                      /country/i.test(label)
                    ) {
                      try {
                        const regionNames = new Intl.DisplayNames(["en"], { type: "region" });
                        const fullName = regionNames.of(displayVal);
                        if (fullName) displayVal = fullName;
                      } catch {
                        /* ignore */
                      }
                    }
              ...
              {useLegacyRegistrationLayout &&
                ...
                Object.keys(data.customData).length > 0 && (
                  <BlockStack gap="300">
                    ...
                    {Object.entries(data.customData as Record<string, unknown>).map(([key, value]) =>
                      renderOneCustomDataEntry(key, value)
                    )}
                  </BlockStack>
                )}
              {!useLegacyRegistrationLayout && orphanCustomDataEntries.length > 0 && (
                <BlockStack gap="300">
                  <Text as="p" variant="bodySm" fontWeight="semibold">
                    Additional saved fields
                  </Text>
                  {orphanCustomDataEntries.map(([key, value]) => renderOneCustomDataEntry(key, value))}
                </BlockStack>
              )}
