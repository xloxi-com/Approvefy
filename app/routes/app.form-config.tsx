...
import { EditIcon, DeleteIcon, PlusIcon, ClipboardIcon, DuplicateIcon, CheckIcon } from "@shopify/polaris-icons";
...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    if (intent === "setDefault") {
        ...
        try {
            ...
            const updated = await prisma.formConfig.updateMany({
                where: { id: formId, shop },
                data: { isDefault: true },
            } as never);
            if (updated.count === 0) return { success: false, error: "Form not found" };
            return { success: true, defaultFormId: formId };
        } catch (e) {
            console.error("Set default form failed:", e);
            return { success: false, error: "Failed to set default form" };
        }
    }
    if (intent === "delete") {
        const formId = formData.get("formId") as string | null;
...
export default function FormConfig() {
    ...
    const actionData = useActionData<{ success?: boolean; clonedFormId?: string; defaultFormId?: string; error?: string }>();
    ...
    useEffect(() => {
        if (actionData?.success && actionData?.defaultFormId) {
            revalidator.revalidate();
            setDefaultToast(true);
        }
    }, [actionData?.success, actionData?.defaultFormId, revalidator]);
            ...
            <IndexTable.Cell>
                <div style={{ display: "flex", gap: "2px", alignItems: "center", justifyContent: "center", width: "100%" }}>
                    <Button
                        variant="plain"
                        icon={CheckIcon}
                        accessibilityLabel={form.isDefault ? "Default form" : "Set as default form"}
                        disabled={form.isDefault}
                        onClick={form.isDefault ? undefined : () => handleSetDefaultForm(form)}
                    />
                    <Button
                        variant="plain"
                        icon={EditIcon}
    ...
    return (
        <Frame>
            ...
            {defaultToast && (
                <Toast content="Default form updated." onDismiss={() => setDefaultToast(false)} />
            )}
            <Page
                        ...
                        {forms.length > 0 && (
                            <Box paddingBlockEnd="300">
                                <Banner tone="info">
                                    <BlockStack gap="200">
                                        <Text as="p">
                                            Theme app extension: in <strong>App embeds</strong>, enable <strong>Approvefy</strong>, open its settings, and turn on <strong>Enable custom registration form</strong>. Optionally paste a Form ID into &quot;Form to display&quot; for a specific form; leave blank for your default form.
                                        </Text>
                                        <Text as="p">
                                            Save the theme after changing embed settings. Use <strong>Customize theme</strong> below to open the editor on the customer register template.
                                        </Text>
                                        <InlineStack gap="200">
