...
export type ThemeSettings = {
  ...
  cardText: string;
  headingColor: string;
  /** Muted subtitle under the storefront heading (`.form-description`). */
...
function normalizeTextAlign(raw: unknown, fallback: TextAlignOption): TextAlignOption {
  if (raw === "left" || raw === "center" || raw === "right") return raw;
  if (typeof raw === "string") {
    const v = raw.trim().toLowerCase();
    if (v === "left" || v === "center" || v === "right") return v as TextAlignOption;
  }
  return fallback;
}
...
export function normalizeThemeSettings(raw: unknown): ThemeSettings {
  ...
  return {
    ...
    formDescriptionColor: normalizeHex(
      o.formDescriptionColor,
...
#custom-registration-container h2 {
  ...
  text-align: ${formTitleTextAlign} !important;
}
...
#custom-registration-container .custom-form-field .custom-help-text {
  color: ${formDescriptionColor} !important;
}

#custom-registration-container .custom-radio-item,
#custom-registration-container .custom-checkbox-item {
  color: ${cardText};
}
