(function() {
  ...
  if (!cfg.enableCustomRegistration) return;
  ...
  var embedFormId = (typeof cfg.embedFormId === 'string') ? cfg.embedFormId : '';
  ...
  function ensureContainerInjected(mainContent, existingRegisterForm) {
    var c = document.getElementById('custom-registration-container');
    if (c) return c;
    var placeholder = document.createElement('div');
    placeholder.id = 'custom-registration-container';
    placeholder.className = 'custom-registration-container';
    placeholder.setAttribute('data-state', 'loading');
    placeholder.innerHTML = '<div class="custom-registration-placeholder" aria-label="Loading"><div class="loader-spinner-circle" role="progressbar" aria-hidden="true"></div><span class="loader-label">' + escapeHtml(t('loading')) + '</span></div>';
    if (existingRegisterForm && existingRegisterForm.parentNode) {
      existingRegisterForm.parentNode.insertBefore(placeholder, existingRegisterForm);
    } else {
  ...
  async function init(shopDomain) {
    // Find the main content area (try theme-specific selectors first) and the existing Shopify registration form
    const mainContent = document.querySelector('#MainContent') || document.querySelector('main.main-content') || document.querySelector('main') || document.querySelector('[role="main"]') || document.querySelector('.main-content') || document.body;
    const existingRegisterForm = mainContent.querySelector('form[action*="/account"]');

    // Inject container immediately with loading state so something is always visible on /account/register
    let container = ensureContainerInjected(mainContent, existingRegisterForm);
    ...
    // If no active configuration, show fallback message and leave default form visible
    if (!hasConfig) {
      var notConfiguredHtml = '<div id="custom-registration-container" class="custom-registration-container" data-state="not-configured">' +
        '<div class="custom-registration-placeholder" style="padding:24px;max-width:560px;margin:0 auto;text-align:center;color:#111;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0;">' +
        '<p style="margin:0 0 12px;font-size:18px;font-weight:600;">' + escapeHtml(t('formNotConfigured')) + '</p>' +
        '<p style="margin:0;font-size:14px;color:#475569;">Configure your registration form in the Approvefy app (Form configuration).</p>' +
        '</div></div>';
      container.outerHTML = notConfiguredHtml;
      return;
    }

    // We have a configured form: enable our custom experience and hide default Shopify form
