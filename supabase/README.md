...
- **DATABASE_URL** – used by Prisma for queries and migrations (set in Vercel / `.env`)
...
- `DATABASE_URL` – from Supabase: **Project Settings → Database → Connection string (URI)**. For pooled connections (PgBouncer), use **Session mode** if `prisma migrate deploy` fails during deploy.
